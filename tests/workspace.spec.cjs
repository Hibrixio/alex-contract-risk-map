...
test('single upload, requested focus, text highlights and task checklist survive reanalysis',async({page})=>{
 ...
 await page.locator('#taskView').click();await expect(page.locator('.task-map')).toBeVisible();await expect(page.locator('.task-map-core')).toContainText('Task map');await expect(page.locator('#taskConnections line')).toHaveCount(1);await page.locator('[data-task]').click();await expect(page.locator('.task-editor h2')).toHaveText('Independent task');
 expect(errors).toEqual([]);
});
test('finance receipt register reads spreadsheets and stores receipt evidence',async({page})=>{
 await signedIn(page);await page.locator('.workspace-nav [data-page=finance]').click();await expect(page.locator('#financePage h1')).toHaveText('Finance');
 await page.locator('#financeSheetFile').setInputFiles({name:'receipts.csv',mimeType:'text/csv',buffer:Buffer.from('Vendor,Paid date,Amount,Receipt\nAcme Supplies,2026-09-10,42.50,acme.pdf')});await expect(page.locator('.finance-table tbody tr')).toHaveCount(1);await expect(page.locator('[data-finance-field="vendor"]')).toHaveValue('Acme Supplies');await expect(page.locator('[data-finance-field="amount"]')).toHaveValue('42.50');
 await page.locator('#financeReceipt').setInputFiles({name:'lunch.png',mimeType:'image/png',buffer:Buffer.from('receipt fixture')});await expect(page.locator('.finance-table tbody tr')).toHaveCount(2);await expect(page.locator('.finance-table')).toContainText('lunch.png');
 await page.locator('.workspace-nav [data-page=tasks]').click();await expect(page.locator('.task-board-table')).toBeVisible();
});
