...
let busy = false, config = {}, currentUser = null, documentKey = '', taskItems = [], taskSelected = '', taskFilter = 'all', taskQuery = '', taskMap = false, taskMapZoom = 1;
...
function taskMapPosition(index,total){const angle=(-Math.PI/2)+(index/Math.max(total,1))*Math.PI*2;return {x:50+Math.cos(angle)*34,y:50+Math.sin(angle)*34};}
function ensureTaskMapPositions(items){let changed=false;items.forEach((task,index)=>{if(!Number.isFinite(Number(task.x))||!Number.isFinite(Number(task.y))||(task.x===50&&task.y===20)){const point=taskMapPosition(index,items.length);task.x=point.x;task.y=point.y;changed=true;}});if(changed)saveTasks();}
function renderTasks(){
  ...
  $('taskAssistantForm').onsubmit=async e=>{e.preventDefault();const input=$('taskAssistantInput'),text=input.value.trim();if(!text)return;input.disabled=true;const history=[...(window.orbitTaskAssistantMessages||[]),text];let reply='I’m thinking…';try{const response=await fetch('/api/task-assistant',{method:'POST',headers:{'Content-Type':'application/json',...await orbitAuthHeaders()},body:JSON.stringify({messages:history,tasks:taskItems.map(t=>({id:t.id,title:t.title,status:t.status,due:t.due,owner:t.owner}))})});const data=await response.json();if(!response.ok)throw Error(data.error||'Assistant unavailable.');reply=data.reply;const target=data.task?.id&&taskItems.find(t=>t.id===data.task.id);if(data.operation==='add'){const id=crypto.randomUUID(),task={id,manual:true,title:data.task?.title||text,priority:'medium',status:data.task?.status||'To do',owner:data.task?.owner||'',due:data.task?.due||'',next:'',notes:'',blockers:'',actions:[],x:50,y:20};taskItems.push(task);taskSelected=id;saveTasks();}else if(target){taskSelected=target.id;if(data.operation==='edit'&&data.task.title)target.title=data.task.title;if(data.operation==='complete')target.status='Done';if(data.operation==='delete')taskItems=taskItems.filter(t=>t.id!==target.id);if(data.task.due)target.due=data.task.due;if(data.task.owner)target.owner=data.task.owner;saveTasks();}}catch(error){reply=`I couldn’t reach Orbit’s AI assistant: ${error.message}`;}window.orbitTaskAssistantMessages=[...history,reply].slice(-8);renderTasks();};
  ...
  $('taskView').onclick=()=>{taskMap=!taskMap;taskMapZoom=1;renderTasks();};
...
function drawTaskLines(){const svg=$('taskConnections');if(!svg)return;const list=$('taskList'),rect=list.getBoundingClientRect(),core=list.querySelector('.task-map-core'),cards=[...list.querySelectorAll('[data-task]')];if(!core)return;const a=core.getBoundingClientRect();svg.setAttribute('viewBox',`0 0 ${rect.width} ${rect.height}`);svg.innerHTML=cards.map(card=>{const b=card.getBoundingClientRect();return `<line x1="${a.x+a.width/2-rect.x}" y1="${a.y+a.height/2-rect.y}" x2="${b.x+b.width/2-rect.x}" y2="${b.y+b.height/2-rect.y}" stroke="#6c8eb8" stroke-width="2" stroke-linecap="round"/>`;}).join('');}
